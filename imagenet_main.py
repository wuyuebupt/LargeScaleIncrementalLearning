...
## Don't forget to set the FLAG in resnet.py
# True  : ImageNet1000
# False : ImageNet100
FLAG_1000=True
# FLAG_1000=False
...
### information for imagenet-1000 classes
#_NUM_IMAGES = {
#    'train': 1281167,
#    'validation': 50000,
#}
### information for imagenet-100 classes
#_NUM_IMAGES = {
#    'train': 129359,
#    'validation': 5000,
#}
...
######### Modifiable Settings ##########
nb_val     = 0.1                # Validation samples per class, percentage of all old classes in stored exemplars.
                                # The current implementation use ceiling and flooring to make sure that we will always use less samples,
                                # which might be improved by better sampling code.
if FLAG_1000 == True:
  nb_cl      = 100              # Classes per group for ImageNet-1000
else:
  nb_cl      = 10               # Classes per group for ImageNet-100 
nb_groups  = 10                 # Number of groups
nb_proto   = 20                 # Number of prototypes per class: total protoset memory/ total number of classes
if FLAG_1000 == True:
  data_dir   = './dataImageNet1000/'   # folder for imagenet-1000 data
else:
  data_dir   = './dataImageNet100/'    # folder for image-100 data
np.random.seed(1993)            # Fix the random seed, for class order, same with iCaRL
########################################
assert (_NUM_CLASSES == nb_cl * nb_groups)
...
x_train, y_train = utils_imagenet.load_data(fpath, order)
fpath            = os.path.join(data_path, 'val.txt')
x_test, y_test   = utils_imagenet.load_data(fpath, order)
...
max_val = int(nb_val * nb_groups * nb_cl * nb_proto / nb_cl)
...
def main(unused_argv):
  ...
  for itera in range(nb_groups):
    ...
    if itera == 0:
      ...
      nb_val_itera  = int(np.floor(nb_val * nb_groups * nb_proto / (itera)))
      nb_protos_cl  = int(np.ceil(nb_proto*nb_groups*1./itera)) - nb_val_itera # Reducing number of exemplars of the previous classes
      # train data
      x_train_from_cl = x_train[itera][:]
      y_train_from_cl = y_train[itera][:]
    ...
    if itera == 0:
      ...
      x_train_resnet_features, beta, gamma, test_accuracy = resnet.resnet_main(FLAGS, imagenet_model_fn, input_fn, x_train_from_cl, y_train_from_cl, x_val_from_cl, y_val_from_cl, x_test_from_cl, y_test_from_cl, itera, nb_groups, 1.0, 0.0)
    else:
      print ("incremental iteras, #training: {}, #val: {}, #test: {}".format(len(x_train_from_cl), len(x_val_from_cl), len(x_test_from_cl)))
      x_train_resnet_features, beta, gamma, test_accuracy = resnet.resnet_main(FLAGS, imagenet_model_fn, input_fn, x_train_from_cl, y_train_from_cl, x_val_from_cl, y_val_from_cl, x_test_from_cl, y_test_from_cl, itera, nb_groups, beta_all[itera-1], gamma_all[itera-1])
    ...
    nb_val_next_itera = int(np.ceil(nb_val * nb_groups * nb_proto / (itera+1)))
    nb_protos_cl  = int(np.ceil(nb_proto*nb_groups*1./(itera+1))) - nb_val_next_itera
